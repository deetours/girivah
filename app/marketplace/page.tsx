    ...
    <section ref={containerRef} className="py-40 bg-[#050505] relative overflow-hidden z-20">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 space-y-32">
        ...
        <motion.div style={{ x: x1 }} className="relative w-[80vw] md:w-[50vw] aspect-[16/9] ml-auto">
          <Image src="/hero-cinematic.jpg" alt="Expedition Visual" fill sizes="(max-width: 768px) 80vw, 50vw" className="object-cover opacity-80" />
        ...
        <motion.div style={{ x: x2 }} className="relative w-[85vw] md:w-[60vw] aspect-[21/9]">
          <Image src="/exp-spiti.jpg" alt="Expedition Visual" fill sizes="(max-width: 768px) 85vw, 60vw" className="object-cover opacity-80" />
        ...
        <motion.div style={{ x: x3 }} className="relative w-[70vw] md:w-[45vw] aspect-[4/3] mx-auto">
          <Image src="/exp-ladakh.jpg" alt="Expedition Visual" fill sizes="(max-width: 768px) 70vw, 45vw" className="object-cover opacity-80" />
...
export default function MarketplaceFallbackPage() {
  return (
    <main className="bg-background text-foreground selection:bg-accent selection:text-white">
      ...
      {/* ═ SCENE 2: THE INFRASTRUCTURE (Ecosystem Stack) ═ */}
      <InfrastructureScene />

      {/* ═ SCENE 3: CALL OF ADVENTURE ═ */}
      <AdventureGalleryScene />

      {/* ═ SCENE 3.5: ROUTE BOARD (Marketplace Discovery) ═ */}
      <section className="py-24 bg-background border-t border-white/5 relative z-20">
         <div className="max-w-[1400px] mx-auto px-6 md:px-12 mb-12">
            <h2 className="font-display text-4xl text-white">Active Manifests.</h2>
            <p className="font-sans text-sm text-white/50 mt-4 max-w-xl">Curated combinations of routes, machines, and refuges. Build upon these foundations or assemble your own from the ground up.</p>
         </div>
         <RouteBoard />
      </section>
      ...
      <section className="py-32 md:py-48 bg-secondary overflow-hidden z-20 relative">
        ...
        {/* The Carousel */}
        <div className="flex gap-6 overflow-x-auto pb-20 px-6 md:px-12 snap-x snap-mandatory hide-scrollbar">
          {EXPEDITIONS.map((exp, i) => (
            <Link href={`/expeditions/${exp.slug}`} key={exp.id} className="w-[85vw] md:w-[35vw] flex-shrink-0 snap-center group relative block">
              <div className="aspect-[4/5] relative overflow-hidden bg-[#0A0A0A] transition-transform duration-700 group-hover:scale-[1.02]">
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end z-10">
                  <div className="flex items-center gap-3 font-sans text-[9px] tracking-[0.25em] uppercase text-white/50 mb-4 pb-4 border-b border-white/10">
                    <span className="flex items-center gap-1.5 text-accent"><Triangle size={6} className="fill-accent stroke-none" />{exp.maxElevation}</span>
                    <span>·</span>
                    <span>{exp.type}</span>
                    <span>·</span>
      ...
      <section className="py-32 bg-[#0A0A0A] border-y border-white/5 relative z-20">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-16">
          <div className="w-full md:w-1/2 aspect-square md:aspect-[4/5] relative overflow-hidden bg-[#111]">
            <Image src="/exp-spiti.jpg" alt="Guidance" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover grayscale opacity-70" />
            <div className="absolute inset-0 bg-black/20" />
            {/* Corner brackets */}
