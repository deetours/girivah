...
import { Expedition } from '@/lib/data/expeditions'
import { AddToTripButton } from '@/components/marketplace/AddToTripButton'

export function ExpeditionDetailView({ expedition }: { expedition: Expedition }) {
            ...
            <div className="sticky top-32 border border-white/10 bg-[#0A0A0A] p-10 md:p-14 relative overflow-hidden group">
              ...
              <div className="flex flex-col gap-4">
                <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}>
                  <Link href={`/booking?item=${expedition.slug}&kind=${expedition.kind}`} className="btn-accent w-full justify-center py-5 block text-center">
                    Apply for this Route
                  </Link>
                </motion.div>
                <AddToTripButton item={expedition} className="w-full justify-center" />
              </div>
